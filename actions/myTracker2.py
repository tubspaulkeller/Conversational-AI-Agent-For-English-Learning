...
def check_if_tracker_store_async(tracker_store) -> bool:
...
class TrackerStore:
    ...
    def __init__(
        ...
        **kwargs: Dict[Text, Any],
        ...
        self.max_event_history: Optional[int] = 100
...
class RedisTrackerStore(TrackerStore, SerializedTrackerAsText):
    ...
    def __init__(
        ...
        **kwargs: Dict[Text, Any],
        ...
        if key_prefix:
            logger.debug(
                f"Setting non-default redis key prefix: '{key_prefix}'.")
...
class MyCustomTracker(TrackerStore, SerializedTrackerAsText):
...
def create_engine_kwargs(url: Union[Text, "URL"]) -> Dict[Text, Any]:
    ...
    kwargs["max_overflow"] = int(
        os.environ.get(POSTGRESQL_MAX_OVERFLOW,
...
def validate_port(port: Any) -> Optional[int]:
    ...
    if port is not None and not isinstance(port, int):
        try:
            ...
            raise RasaException(
                f"The port '{port}' cannot be cast to integer.") from e
...
class SQLTrackerStore(TrackerStore, SerializedTrackerAsText):
    ...
    class SQLEvent(Base):
        ...
        id = sa.Column(sa.Integer, _create_sequence(
        ...
        **kwargs: Dict[Text, Any],
        ...
        while True:
            try:
                ...
                self.sessionmaker = sa.orm.session.sessionmaker(
                    bind=self.engine)
                break
            except (
                sqlalchemy.exc.OperationalError,
    ...
    def _create_database(engine: "Engine", database_name: Text) -> None:
        ...
        if not matching_rows:
            try:
                ...
                logger.error(
                    f"Could not create database '{database_name}': {e}")
    ...
    def _event_query(
        ...
        session_start_sub_query = (
            session.query(sa.func.max(
                self.SQLEvent.timestamp).label("session_start"))
            .filter(
    ...
    async def save(self, tracker: DialogueStateTracker) -> None:
        ...
        with self.session_scope() as session:
            ...
            for event in events:
                data = event.as_dict()
                intent = (
                    data.get("parse_data", {}).get(
                        "intent", {}).get(INTENT_NAME_KEY)
                )
    ...
    def _additional_events(
        ...
        return itertools.islice(
            tracker.events, number_of_events_since_last_session, len(
                tracker.events)
        )
...
def _create_from_endpoint_config(
    ...
    if endpoint_config is None or endpoint_config.type is None:
        ...
        tracker_store: TrackerStore = InMemoryTrackerStore(
            domain, event_broker)
